...
export const ShortcutInput: React.FC<ShortcutInputProps> = ({
  ...
  const cancelRecording = useCallback(async () => {
    ...
    await commands.stopKandyKeysRecording().catch(console.error);
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      ...
      const unlisten = await listen<KandyKeysEvent>(
        "handy-keys-event",
        async (event) => {
          if (cleanup) return;
    ...
    return () => {
      ...
      commands.stopKandyKeysRecording().catch(console.error);
    };
  }, [
    isRecording,
    shortcutId,
  ...
  const startRecording = async () => {
    ...
    try {
      const result = await commands.startKandyKeysRecording(shortcutId);
