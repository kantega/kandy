...
// Native overlay window size (logical points). One window is reused for every
// state; it need only be at least as large as the card it hosts (the `--ov-*`
// vars in RecordingOverlay.css). The card is CSS-anchored flush to the screen
// edge, so window height doesn't move where the card sits, only
// OVERLAY_TOP_OFFSET / OVERLAY_BOTTOM_OFFSET do. Keep these in sync with the
// CSS card geometry.
//
// The 40h pill animates width from 172 (--ov-rest-w) to 216 (--ov-work-w) and
// expands from center, so the window must fit the widest state plus slack.
...
/// Returns overlay position in logical coordinates (points).
///
...
fn show_overlay_state(app_handle: &AppHandle, state: &str) {
    ...
    // Hop to the main thread so NSScreen access stays main-thread
    // correct. run_on_main_thread runs the closure inline when already on the
    // main thread, so this never deadlocks.
...
fn show_overlay_state_on_main(app_handle: &AppHandle, state: &str) {
    // A show supersedes any pending delayed hide from a previous
...
/// first sample chunk. Skipped when overlays are disabled.
pub fn emit_recording_ready(app_handle: &AppHandle) {
    ...
    // there as well so a very fast stream cannot overtake show-overlay and then
    // get reset back to the arming state by the frontend.
    let handle = app_handle.clone();
...
pub fn update_overlay_position(app_handle: &AppHandle) {
    // Positioning queries monitors/cursor and moves the window, so it runs on
    // the main thread (see show_overlay_state).
    let handle = app_handle.clone();
...
/// Bumped by every show and hide. The delayed hide only fires when no show (or
/// later hide) has happened in the meantime, so a recording started within the
/// 300 ms fade-out window keeps its overlay.
static HIDE_GENERATION: AtomicU64 = AtomicU64::new(0);

/// Hides the recording overlay window with fade-out animation
pub fn hide_recording_overlay(app_handle: &AppHandle) {
    // Always hide the overlay regardless of settings: if the setting was changed
    // while recording, the overlay still has to go away.
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        let _ = overlay_window.emit("hide-overlay", ());
        let generation = HIDE_GENERATION.fetch_add(1, Ordering::SeqCst) + 1;
        // Hide the window after a short delay to allow the animation to complete
...
pub fn emit_levels(app_handle: &AppHandle, levels: &[f32]) {
    ...
    // guard a hidden overlay's WebKit subprocess still processes every event,
    // and each event drives a WebKit allocation that accumulates without bound
    // (see issue #1279 for the investigation).
    if !OVERLAY_ENABLED.load(Ordering::Relaxed) {
        return;
    }
