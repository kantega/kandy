...
pub enum VadPolicy {
    ...
    Disabled,
    /// Keep only detected speech (plus pre-roll and hangover).
    Enabled,
}

type SharedVad = Arc<Mutex<Box<dyn vad::VoiceActivityDetector>>>;
...
struct CaptureProcessor {
    ...
    in_sample_rate: u32,
    /// Samples per detector frame, so tail diagnostics report real milliseconds.
    frame_samples: usize,
    vad: Option<SharedVad>,
    level_cb: Option<LevelCallback>,
...
impl CaptureProcessor {
    fn new(
        ...
        let frame_samples = vad.as_ref().map_or(
            ...
            |detector| detector.lock().unwrap().frame_samples(),
        );
        let frame_duration =
