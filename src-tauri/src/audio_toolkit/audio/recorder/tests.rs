use super::{
    ...
    CaptureProcessor, CaptureTransportState, ChunkDisposition, Cmd, VadPolicy,
};
...
/// Pass-through detector that records the frame sizes it is handed, standing
/// in for a backend such as Earshot whose frames are not 30 ms.
struct FrameSizeSpy {
    frame_samples: usize,
    seen: Arc<Mutex<Vec<usize>>>,
}

impl VoiceActivityDetector for FrameSizeSpy {
    fn push_frame<'a>(&'a mut self, frame: &'a [f32]) -> anyhow::Result<VadFrame<'a>> {
        self.seen.lock().unwrap().push(frame.len());
...
/// The resampler must emit frames sized for the active detector, or the
/// detector rejects every frame as the wrong length.
#[test]
fn resampler_frame_size_follows_the_vad_backend() {
    let frame_samples = 256;
    let seen = Arc::new(Mutex::new(Vec::new()));
    let detector: Box<dyn VoiceActivityDetector> = Box::new(FrameSizeSpy {
        frame_samples,
        seen: Arc::clone(&seen),
    });
...
fn repeated_start_stop_cycles_resume_capture_without_leaking_samples() {
    ...
    let worker = thread::spawn(move || {
        let processor = CaptureProcessor::new(16_000, None, None, Instant::now());
        run_consumer(
            processor,
            consumer,
            cmd_rx,
