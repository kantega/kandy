...
/// Filler tokens removed from every transcription. Kandy transcribes
/// Norwegian and English, so the list covers both plus tokens that are not
/// lexical words in either. Anything that is a real word in one of them
/// ("mm" millimetres, "ha" Norwegian imperative) stays out.
const FILLER_WORDS: &[&str] = &[
    "uh", "uhm", "umm", "uhh", "uhhh", "ehh", "ehm", "ahm", "hmm", "hm", "mmm", "um", "eh", "ah",
    "øh", "øhm", "ehm", "æh", "æhm", "hmmm",
];
...
pub fn remove_filler_words(text: &str, enabled: bool) -> String {
    if !enabled {
        return text.to_string();
    }
...
mod tests {
    ...
    /// Exercise the complete cleanup sequence.
    fn filter_transcription_output(text: &str) -> String {
        normalize_transcription_output(&remove_filler_words(text, true))
    }
    ...
    #[test]
    fn test_filter_norwegian_fillers() {
        assert_eq!(
            filter_transcription_output("øh jeg tror øhm at det, eh, fungerer"),
            "jeg tror at det, fungerer"
        );
    }

    #[test]
    fn test_filter_master_toggle_disables_removal() {
        let text = "um I think";
        assert_eq!(remove_filler_words(text, false), text);
    }

    #[test]
    fn test_filter_preserves_millimetre_unit() {
        // "mm" must not be a filler word: it is also a unit.
    ...
    fn test_apply_custom_words_handles_unicode_punctuation() {
        let text = "「Kandee。」";
        let custom_words = vec!["Kandy".to_string()];
        let result = apply_custom_words(text, &custom_words, 0.5);
        assert_eq!(result, "「Kandy。」");
    }
