...
impl VoiceActivityDetector for SmoothedVad {
    ...
    /// Trailing run of withheld frames plus smoothing state. Interior withheld
    /// frames (before already-emitted speech) are not counted.
...
mod tests {
    ...
    fn tail_report_counts_withheld_onset_tail() {
        // One voiced frame at the end: onset (2 frames) never confirms, so both
        // trailing frames are withheld and one of them is voiced, consistent
        // with a final word cut off at the stop boundary.
    ...
    fn tail_report_counts_only_trailing_run() {
        ...
        // withheld. Only the trailing run counts, so frames older than an
