...
fn classify_busy_input(
    ...
    match (mode, is_pressed, remembered) {
        ...
        // Hold modes: a press while busy means the user is holding the key, so
...
/// `held_binding` is the binding whose key we believe is down, either the
/// one recording, or the one remembered while the pipeline is busy.
...
impl CoordinatorState {
    ...
    /// holding, either the live recording or a press remembered while busy.
...
/// state machine can roll back its optimistic transition on failure. A start
/// is refused when the recording belongs to someone else (for example while a
...
mod tests {
    ...
    /// Hold-or-toggle: a key held past the threshold is push-to-talk, so the
    ...
    /// only ~100ms after the drain, so it is a hold and must stop, not lock.
    #[test]
