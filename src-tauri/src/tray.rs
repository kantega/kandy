...
//! disappearance bug (tauri-apps/tauri#12060). Before this change, every
...
pub fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("Kandy v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("Kandy v{}", env!("CARGO_PKG_VERSION"))
    }
}

/// Builds the tray menu and tooltip for the given inputs. Pure with respect to
/// app state: everything it depends on is in `inputs`.
fn build_menu(app: &AppHandle, inputs: &MenuInputs) -> tauri::Result<(Menu<tauri::Wry>, String)> {
    ...
    let menu = if inputs.busy {
        ...
        // Build model submenu; the label is the active model name.
...
/// tooltip. Called when the user "relaunches" Kandy while it is already running
/// (`RunEvent::Reopen` for Spotlight/Finder/Dock, the single-instance callback
/// for a second process), which is the natural "where did my icon go?" moment,
/// so a relaunch brings the icon back without a full quit.
...
mod tests {
    ...
    fn build_entry(transcription: &str, post_processed: Option<&str>) -> HistoryEntry {
        HistoryEntry {
            id: 1,
            file_name: "kandy-1.wav".to_string(),
            timestamp: 0,
            saved: false,
